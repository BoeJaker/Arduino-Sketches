...
// Your Wifi Network
String wifi_ssid[] = {"Galaxy Note", "BTHub6-W7TF"};
String wifi_password[] ={"adalovelace", "NbyCy4Q3RKhk"} ;
...
void wifiConnect(){
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    tft->drawString(wifi_ssid[ssid], 150, 25, 2);
    WiFi.begin(wifi_ssid[ssid], wifi_password[ssid]);
    int counter = 0;
    while (WiFi.status() != WL_CONNECTED) {
      if (counter >= 5000){
        WiFi.mode(WIFI_OFF);
        WiFi.begin(wifi_ssid[ssid], wifi_password2);
